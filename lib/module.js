...
var Module = (function (_super) {
    ...
    Module.prototype.getAvailabilities = function (data, callback) {
        ...
        this.db.getConnection(function (err, connection, cleanup) {
            ...
            var queryParams = [];
            var query = connection.query("SELECT * FROM availability " + "WHERE (CASE WHEN ? IS NOT NULL THEN idUser = ? ELSE true END) " + "AND (CASE WHEN ? IS NOT NULL THEN startDate >= ? ELSE true END) " + "AND (CASE WHEN ? IS NOT NULL THEN endDate <= ? ELSE true END) " + "ORDER BY startDate, endDate", [data.idUser, data.idUser, data.startDate, data.startDate, data.endDate, data.endDate], function (err, rows) {
                cleanup();
                if (err) {
                    return callback(new DatabaseError(err));
                }
